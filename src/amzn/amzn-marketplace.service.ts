...
export class AmznMarketplaceService {
  ...
  private readonly marketplaceParticipationUrl =
  ...
  findOne(filter: Partial<MarketplaceEntry>): MarketplaceEntry | undefined {
    return this.marketplaceEntries.find((entry) => {
      return Object.keys(filter).every((key) => {
        const entryValue = entry[key as keyof MarketplaceEntry];
        const filterValue = filter[key as keyof MarketplaceEntry];
